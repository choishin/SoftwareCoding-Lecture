 ...
 * => 최종 해결한 방법 1 : 각 물건당 세전가격을 더하는 방식으로는 계산에 오차가 생길 수 있음 (숨겨져있는 소수점 이하의 숫자들)
 *						  과세물품을 더한 총합에서 1.1을 계산해서 총합을 빼주면 정확함.
 * => 최종 해결한 방법 2 : for 반복문 안에 과세금액을 구하는 식을 넣으면 오류가 생기기 쉬움 => 밖으로 빼줌
 * 
 * 
 */
...
public class K40_Main {
	...
	public static void main(String[] args) throws ParseException {
		...
		SimpleDateFormat K40_format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); //Calendar 클래스에서 시스템에서 날짜정보를 받아오기 위해 오브젝트 생성 (영수증 하단 주차시간 부분)
		SimpleDateFormat K40_format2 = new SimpleDateFormat("yyyy-MM-dd HH:mm"); //SimpleDataFormat 클래스에서 받아온 날짜정보를 출력할 포맷 생성 (영수증 상단 왼쪽)
		SimpleDateFormat K40_format3 = new SimpleDateFormat("yyyyMMdd"); //SimpleDataFormat 클래스에서 받아온 날짜정보를 출력할 포맷 생성 (영수증 하단부 바코드 부분)
		...
		String[] K40_convertItemNames = convertNames(K40_items); //1-(1). 상품이름을 'convertname'이라는 메소드로 보내 17byte+" "으로 만들어 main으로 돌아와 새로운 배열에 넣어줌.
		...
		int K40_count = 0; //총 항목의 갯수 (5줄마다 ---을 그어주기 위함)
		int K40_taxfreeTotal = 0; //면세물품의 총합
		int K40_nontaxfreeTotal = 0; //과세물품의 총합
		double K40_tax = 0.1; //세율
		int K40_netprice = 0; //세전가격 
		int K40_taxTotal = 0; //세금총합
		int K40_total = 0; //총합계 금액
		
		int point ; //point
		int oldPoint = 5637; //이전 포인트
		int totalAmount = K40_items.length; //상품목록 갯수 
		...
		System.out.println();
		System.out.printf("%21.21s %17.17s\n","[구 매]"+K40_format2.format(K40_calendar2.getTime()),"POS:0011-9861"); //(년월일을 '-'으로 구분하고, 시분만 표현 yyyy-MM-dd HH:mm)
		System.out.printf("-----------------------------------------\n");
