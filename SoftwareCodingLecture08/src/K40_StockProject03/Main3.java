...
public class Main3 {
	...
	public static void main(String[] args) throws IOException {
		...
		File K40_f1 = new File("C:\\Users\\최신\\Desktop\\day_data\\A005930_minmax.csv"); 
		...
		while((K40_readtxt= K40_br.readLine()) !=null){ //(3) while반복문 (무한반복)
			...
			//if 조건문 : 2번째 필드부터확인 시작, field[1]항목에 포함된 '^'를 ""으로 바꿔주고, 공백은 없애고 '2015'가 포함된 항목이 있고,
			// field[2]항목에 포함된 '^'를 ""으로 바꿔주고, 공백은 없애고 항목에 'A005930'가 포함된 항목이 있다면
			if(K40_field.length > 2 && K40_field[1].replace("^","").trim().contains("2015")&& K40_field[2].replace("^","").trim().contains("A005930")) {
